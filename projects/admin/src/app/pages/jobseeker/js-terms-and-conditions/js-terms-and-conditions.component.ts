...
export class JsTermsAndConditionsComponent implements OnInit {
	...
	getTC() {
		this.textMode = 'view';
		this.busy = this._httpService.getJobseekersTC({ 'id': 3 })
			.subscribe(
				response => {
					if (response.success) {
						let terms = atob(response.result.body);
	...
	saveTC() {
		...
		this.busy = this._httpService.updateJobseekersTC({ 'id': 3, 'title': 'aa', 'body': btoa(this.terms) })
			.subscribe(
				response => {
					if (response.success) {
