...
export class AdminPaymentComponent implements OnInit {
	...
	getSumOfEmpNormalWorkHrSalary() {
		...
		return this.displayDatasource.map(t => t).reduce((previous, current) => {
			return (current.normal_salary_type == 1) ? Number(previous) + Number(current.employer_charge) : Number(previous);
		}, 0);
	}
	...
	getSumOfEmpTotalWorkHrSalary() {
		...
		// }, 0);
		return this.displayDatasource.map(t => t).reduce((previous, current) => {
			return Number(previous) + (Number(current.employer_charge) + Number(current.employer_ot_charge));
		}, 0);
	}
	...
	getSumOfEmpOT1point5WorkHrSalary() {
		...
		// }, 0);
		return this.displayDatasource.map(t => t).reduce((previous, current) => {
			return (current.ot_salary_type == 1.5) ? (current.normal_salary_type == 1.5) ? Number(previous) + ((Number(current.employer_charge)) + (Number(current.employer_ot_charge))) : Number(previous) + Number(current.employer_ot_charge) : (current.normal_salary_type == 1.5) ? Number(previous) + Number(current.employer_charge) : Number(previous);
		}, 0);
	}
	...
	getSumOfEmpOT2WorkHrSalary() {
		...
		// }, 0);
		return this.displayDatasource.map(t => t).reduce((previous, current) => {
			return (current.ot_salary_type == 2) ? (current.normal_salary_type == 2) ? Number(previous) + ((Number(current.employer_charge)) + (Number(current.employer_ot_charge))) : Number(previous) + Number(current.employer_ot_charge) : (current.normal_salary_type == 2) ? Number(previous) + Number(current.employer_charge) : Number(previous);
		}, 0);
	}
