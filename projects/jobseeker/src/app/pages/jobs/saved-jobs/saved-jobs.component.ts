...
export class SavedJobsComponent implements OnInit, OnDestroy {
				...
				response => {
					if (response.success) {
						if ((response.result).length > 0) {
							let savedJobs = response.result.filter(job => job.saved);
							this.jobs = savedJobs;
							this.saved_jobs_list = savedJobs;
	...
	jobSearch() {
		setTimeout(() => {
			...
			// }
			this.saved_jobs_list = [];
			this.saved_jobs_list = this.jobs.filter((jobs: any) => {
			...
			if (this.search.jobspecialization.length > 0) {
				this.saved_jobs_list = this.saved_jobs_list.filter((job: any) => {
